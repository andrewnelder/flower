...
class LoginHandler(BaseHandler, GoogleOAuth2Mixin):
    ...
    def _on_auth(self, response):
        ...
        if not re.match(self.application.auth, user_data.get('email')):
            raise tornado.web.HTTPError(
                404, (
                    "Access denied to '{email}'. "
                    "Please use another account or ask your admin to "
                    "add your email to flower --auth"
                ).format(user_data.get('email'))
            )

        self.set_secure_cookie("user", str(user_data.get('email')))
